...
| V~OH~ |            输出高电平门限            |
| V~IN~ |            输入高电平门限            |
| V~T~  | 阈值电压（电路刚刚勉强能翻转的电压） |
| V~IL~ |            输入低电平门限            |
| V~OL~ |            输出低电平门限            |

**关系：V~OH~ > V~IH~ > V~T~ > V~IL~ > V~OL~**
...
- **建立时间（T~su~）**：在时钟采样沿之前，数据必须保持稳定的时间，该时间量称为建立时间。

- **保持时间（T~h~）**：在时钟采样沿之后，数据必须保持稳定的最短时间。
...
​					（1 +（0.100_1000_1000_0000_0000_0000）~2~）x 2^6^ =（1 + 0.56640625）~10~ x 64 = 100.25
...
**float类型的1.231转换为8位的定点数，1个符号位，4个整数位，3个小数位。**这个3称为量化系数。以下过程称为**量化**。
						1.231 2^3^ = 9.84（取10）~10~ =（0000_1010）~2~
